...
DATA_LAST = 107         # dong cuoi bang du lieu (100 dong)
...
ws["B2"] = "HƯỚNG DẪN SỬ DỤNG FILE BÁO CÁO DOANH THU - THÔNG TIN ĐƠN HÀNG (THÁNG 7 - THÁNG 12/2026)"
ws["B2"].font = Font(name=FONT, size=14, bold=True, color="1F4E78")

lines = [
    ("1. CẤU TRÚC FILE", True),
    ("   • 6 sheet 'Tháng 7' → 'Tháng 12': nhập đơn hàng của từng tháng, mỗi tháng 1 sheet riêng.", False),
    ...
    ("   • Sheet 'Tháng 7' có sẵn 3 dòng VÍ DỤ MẪU (chữ màu xanh dương) — hãy xóa hoặc ghi đè bằng dữ liệu thật của bạn.", False),
    ("", False),
    ...
    ("   • DOANH THU THEO TUẦN: mỗi tuần (Thứ 2 → Chủ nhật) 1 dòng, tự cộng từ tất cả 6 sheet tháng.", False),
    ...
    ("   • Mỗi sheet tháng có sẵn 100 dòng (dòng 8 → 107) đã cài công thức.", False),
    ("   • Muốn thêm: bôi đen 1 dòng GIỮA bảng (ví dụ dòng 50) → chuột phải → 'Chèn 1 hàng phía trên', rồi kéo công thức cột STT và THÀNH TIỀN từ dòng trên xuống. Mọi công thức báo cáo sẽ tự mở rộng theo.", False),
    ("   • KHÔNG nhập dữ liệu xuống dưới dòng 107 (ngoài bảng) — báo cáo sẽ không cộng phần đó.", False),
    ("", False),
    ("7. QUY ƯỚC MÀU", True),
...
EXAMPLES = [  # chi dat o sheet Thang 7 — du lieu vi du, nguoi dung thay bang du lieu that
    [dt.date(2026, 7, 3), "Nguyễn Văn An", "Công ty TNHH Thương Mại ABC", "0312345678",
     "0901234567", "Bình giữ nhiệt in logo", 100, 85000, "Đã chuyển khoản", "Giao đợt 1 (VÍ DỤ MẪU)"],
    [dt.date(2026, 7, 10), "Trần Thị Bích", "Công ty CP Đầu Tư XYZ", "0109876543",
     "0987654321", "Sổ tay da in logo", 50, 120000, "Chưa chuyển khoản", "(VÍ DỤ MẪU)"],
    [dt.date(2026, 7, 15), "Lê Minh Cường", "Công ty TNHH DEF", "0301122334",
...
c = ws["A1"]; c.value = f"BÁO CÁO TỔNG HỢP DOANH THU NĂM {YEAR} (THÁNG 7 → THÁNG 12)"
c.font = f_title; c.fill = fill_title; c.alignment = center
ws.row_dimensions[1].height = 26
...
ic = ws["C4"]; ic.value = dt.date(YEAR, 7, 15)
...
for rq, (a, b) in ((q3, (m_first, m_first + 2)), (q4, (m_first + 3, m_first + 5))):
...
for col in (7, 8, 9):
    L = get_column_letter(col)
    cc = ws.cell(row=ytot, column=col, value=f"=SUM(${L}${q3}:${L}${q4})")
...
note = ws.cell(row=ytot + 2, column=6,
    value="Ghi chú: file theo dõi từ Tháng 7 đến Tháng 12/2026 nên Tổng năm = Quý 3 + Quý 4. "
